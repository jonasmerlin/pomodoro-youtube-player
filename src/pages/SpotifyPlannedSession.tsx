...
// =============================================================================
// SpotifyPlannedSession — planned interval session with Spotify playback
// =============================================================================
//
// Same interval builder as YouTube Planned, but without the video playlist
// (Spotify's currently-playing music is controlled via API, there's no concept
// of queuing individual tracks per interval block).
//
// Two phases: planning (build interval blocks) → running (timer steps through
// the flattened intervals, Spotify plays during work and pauses during breaks).

const SpotifyPlannedSession: React.FC = () => {
    ...
    return (
      <>
        ...
        <div className="w-full text-center p-8 bg-white rounded-lg border border-slate-200">
          <p className="mb-6 text-slate-600">Connect your Spotify account to use planned sessions.</p>
          ...
          {playbackState?.item ? (
            <div className="flex items-center gap-4">
              {playbackState.item.album?.images?.[0] && (
                <img src={playbackState.item.album.images[0].url} alt="Album Art" className="w-16 h-16 rounded-md" />
              )}
              <div>
                <p className="font-medium text-slate-900">{playbackState.item.name}</p>
                <p className="text-slate-600 text-sm">{playbackState.item.artists.map((a) => a.name).join(", ")}</p>
                <p className="text-xs mt-1.5 inline-block px-2 py-0.5 bg-slate-100 rounded-full text-slate-600">
                  {playbackState.is_playing ? (
                    <span className="flex items-center"><span className="w-2 h-2 bg-green-500 rounded-full mr-1.5 animate-pulse" />Playing</span>
                  ) : "Paused"}
                </p>
              </div>
            </div>
            ...
            <p className="text-center text-slate-500 py-4">
              {playbackState?.message || "No track currently playing"}
            </p>
          )}
        </div>
      </>
    ...
    <>
      ...
      <div className="w-full bg-white p-4 rounded-lg border border-slate-200 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {userInfo?.images?.[0] && (
              <img src={userInfo.images[0].url} alt="Profile" className="w-10 h-10 rounded-full" />
            )}
            <div>
      ...
      <div className="mt-8 p-4 bg-slate-50 rounded-lg border border-slate-200 w-full">
        <p className="text-slate-700">
          <span className="font-semibold">How to use:</span> Add interval blocks, start playing music on Spotify,
          then hit Start Session. Music plays during work and pauses during breaks.
        </p>
      </div>
    </>
  );
...
export default SpotifyPlannedSession;
